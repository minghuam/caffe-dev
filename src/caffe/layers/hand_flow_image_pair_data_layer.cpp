...
HandFlowImagePairDataLayer<Dtype>::~HandFlowImagePairDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void HandFlowImagePairDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

  HandFlowImagePairDataParameter data_param = this->layer_param_.hand_flow_image_pair_data_param();
  const string& source = data_param.source();
  const int batch_size = data_param.batch_size();
  const string& image_mean_file = data_param.image_mean_file();
  const string& hand_mean_file = data_param.hand_mean_file();
  num_stack_frames_ = data_param.num_stack_frames();

  BlobProto blob_proto;
  Blob<Dtype> mean_blob;
  
  ReadProtoFromBinaryFileOrDie(image_mean_file.c_str(), &blob_proto);
  mean_blob.FromProto(blob_proto);
  image_mean_ = cv::Mat::zeros(mean_blob.height(), mean_blob.width(), CV_32FC3);
  for(int c = 0; c < mean_blob.channels(); ++c){
    for(int h = 0; h < mean_blob.height(); ++h){
      for(int w = 0; w < mean_blob.width(); ++w){
        image_mean_.at<cv::Vec3f>(h, w)[c] = mean_blob.data_at(0, c, h, w);
      }
    }
  }
  ...
  LOG(INFO) << "Image mean file: " << image_mean_file;
  LOG(INFO) << "Hand mean file: " << hand_mean_file;
  ...
  while(in_file >> flow_folder >> hand_folder >> image_folder \
    ...
    ls_files(group.hand_images, hand_folder, "jpg");
  ...
  this->transformed_data_.Reshape(1, num_stack_frames_ * 3 + 3, height, width);

  int shape_array[4] = {batch_size, num_stack_frames_ * 3 + 3, height, width};
...
template <typename Dtype>
inline int HandFlowImagePairDataLayer<Dtype>::get_frame_num(const string& path){
...
template <typename Dtype>
void HandFlowImagePairDataLayer<Dtype>::ShuffleImages() {
  ...
  for(int i = 0; i < flow_image_groups_.size(); i++){ 
    ...
    vector<string> &hand_images = flow_image_groups_[i].hand_images;
    ...
    for(int j = 0; j < images.size(); j++){
      ...
      for(int k = 0; k < max_num - min_num + 1; k++){
        ...
        for(int offset = 0; offset < num_stack_frames_; offset++){
          ...
          pair.hand_images.push_back(hand_images[index + offset]);
        }
        pair.image = images[j];
...
template <typename Dtype>
float HandFlowImagePairDataLayer<Dtype>::RandWeight(){
...
template <typename Dtype>
void HandFlowImagePairDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  HandFlowImagePairDataParameter data_param = this->layer_param_.hand_flow_image_pair_data_param();
  const int batch_size = data_param.batch_size();
  ...
  this->transformed_data_.Reshape(1, num_stack_frames_ * 3 + 3, height, width);
  batch->data_.Reshape(batch_size, num_stack_frames_ * 3 + 3, height, width);
  ...
  for(int item_id = 0; item_id < batch_size; item_id++){
    ...
    // pack a datum
    Datum datum;
    datum.set_channels(num_stack_frames_ * 3 + 3);
    datum.set_height(input_height);
    datum.set_width(input_width);
    datum.clear_data();
    datum.clear_float_data();

    cv::Mat Iobj = cv::imread(pair.image);
    ...
    for(int i = 0; i < num_stack_frames_; i++){
        ...
        cv::Mat Ihand = cv::imread(pair.hand_images[i], CV_LOAD_IMAGE_GRAYSCALE);
        ...
        for(int h = 0; h < Ihand.rows; ++h){
          for(int w = 0; w < Ihand.cols; ++w){
            float val = (float)Ihand.at<uchar>(h, w) - hand_mean_.at<float>(h, w);
            datum.add_float_data(val);
          }
        }
    }
    ...
    // rgb image
    for(int c = 0; c < Iobj.channels(); ++c){
      for(int h = 0; h < Iobj.rows; ++h){
        for(int w = 0; w < Iobj.cols; ++w){
          datum.add_float_data((float)Iobj.at<cv::Vec3b>(h, w)[c] - \
                               image_mean_.at<cv::Vec3f>(h, w)[c]);         
        }
      }
    }
        // set data
...
INSTANTIATE_CLASS(HandFlowImagePairDataLayer);
REGISTER_LAYER_CLASS(HandFlowImagePairData);
